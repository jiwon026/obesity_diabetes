    ...
    if is_adult:
        ...
        if "DIABETES" in filtered_df.columns:
            ...
            if len(bmi_plot_df) > 0:
                ...
                for bar, rate, n in zip(bars, y_vals, n_vals):
                    if pd.isna(rate):
                        continue
                    ax_bmi.text(
            ...
            if len(trend_df) > 0:
                ...
                ax2 = ax1.twinx()
                ax2.scatter(
                    years, dm_rate, color="red", marker="s", label="당뇨 유병률 (%)"
                )
                ax2.plot(
                    x_line,
                    dm_line(x_line),
                ...
                fig_trend.suptitle(
                    "연도별 비만율과 당뇨 유병률 추이", fontsize=13, fontweight="bold"
                )
